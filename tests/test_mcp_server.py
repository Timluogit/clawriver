...
class TestMCPServerProtocol:
    ...
    def test_tool_count_is_34(self):
        """验证工具总数为 34"""
        from clawriver_mcp.server import MemoryMarketMCPServer
    ...
    def test_fastmcp_instance_exists(self):
        """验证 FastMCP 实例已创建"""
        from clawriver_mcp.server import mcp
        assert mcp is not None
        assert mcp.name == "ClawRiver"

    def test_config_json_valid(self):
        """验证 config.json 是有效 JSON 且 total_tools=34"""
        config_path = os.path.join(os.path.dirname(__file__), "..", "clawriver_mcp", "config.json")
...
class TestMemoryTools:
    ...
    async def test_search_memories_success(self):
        """搜索记忆 — 成功场景"""
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=MOCK_SEARCH_RESULT):
            from clawriver_mcp.server import search_memories
    ...
    async def test_search_memories_with_filters(self):
        ...
        with patch("clawriver_mcp.server.api_request", mock_fn):
            from clawriver_mcp.server import search_memories
    ...
    async def test_search_memories_error(self):
        """搜索记忆 — API 报错"""
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, side_effect=Exception("timeout")):
            from clawriver_mcp.server import search_memories
    ...
    async def test_get_balance(self):
        """查看余额"""
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=MOOK_BALANCE):
            from clawriver_mcp.server import get_balance
    ...
    async def test_upload_memory(self):
        ...
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=mock_resp):
            from clawriver_mcp.server import upload_memory
    ...
    async def test_purchase_memory_success(self):
        ...
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=mock_resp):
            from clawriver_mcp.server import purchase_memory
    ...
    async def test_rate_memory(self):
        ...
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=mock_resp):
            from clawriver_mcp.server import rate_memory
    ...
    async def test_verify_memory(self):
        ...
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=mock_resp):
            from clawriver_mcp.server import verify_memory
...
class TestTeamTools:
    ...
    async def test_create_team(self):
        """创建团队"""
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=MOCK_TEAM):
            from clawriver_mcp.server import create_team
    ...
    async def test_get_team(self):
        """获取团队详情"""
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=MOCK_TEAM):
            from clawriver_mcp.server import get_team
    ...
    async def test_invite_member(self):
        ...
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=mock_resp):
            from clawriver_mcp.server import invite_member
    ...
    async def test_add_team_credits(self):
        ...
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=mock_resp):
            from clawriver_mcp.server import add_team_credits
    ...
    async def test_transfer_credits(self):
        ...
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=mock_resp):
            from clawriver_mcp.server import transfer_credits
    ...
    async def test_log_activity(self):
        ...
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value=mock_resp):
            from clawriver_mcp.server import log_activity
    ...
    async def test_delete_team_memory(self):
        """删除团队记忆"""
        with patch("clawriver_mcp.server.api_request", new_callable=AsyncMock, return_value={}):
            from clawriver_mcp.server import delete_team_memory
...
class TestFormatting:
    ...
    def test_fmt_search_empty(self):
        from clawriver_mcp.server import fmt_search
    ...
    def test_fmt_content_empty(self):
        from clawriver_mcp.server import fmt_content
    ...
    def test_fmt_trends_empty(self):
        from clawriver_mcp.server import fmt_trends
    ...
    def test_fmt_my_memories_empty(self):
        from clawriver_mcp.server import fmt_my_memories
...
class TestMCPServerWrapper:
    ...
    def test_run_stdio(self):
        ...
        from clawriver_mcp.server import MemoryMarketMCPServer, mcp
...
class TestAPIRequest:
    ...
    async def test_api_request_get(self):
        ...
        with patch("clawriver_mcp.server.httpx.AsyncClient") as MockClient:
            instance = AsyncMock()
            ...
            from clawriver_mcp.server import api_request
...
class TestEnvironmentConfig:
    ...
    def test_default_api_base(self):
        from clawriver_mcp.server import _get_api_base
    ...
    def test_api_key_from_env(self):
        from clawriver_mcp.server import _get_api_key
